...
export default function GruposPage() {
    ...
    const handleSaveGroup = async (groupData: { id?: number; nome: string; descricao?: string | null; }) => {
        try {
            ...
            if (!res.ok) {
                showToast({ type: 'error', message: data.error || `Falha ao ${isEditing ? 'editar' : 'criar'} grupo.` });
                return;
            }

            showToast({ type: 'success', message: `Grupo ${isEditing ? 'editado' : 'criado'} com sucesso.` });
            fetchGroups();
        } catch (err) {
    ...
    const handleDeleteGroup = async (group: Group) => {
        const confirmed = window.confirm(`Tem certeza que deseja deletar o grupo "${group.nome}"?`);
        if (!confirmed) return;
        ...
        try {
            ...
            const data = await res.json().catch(() => ({}));
            if (!res.ok) {
                showToast({ type: 'error', message: data.error || 'Falha ao deletar grupo.' });
                return;
            }
    ...
    const handleManageContactsClick = async (group: Group) => {
        try {
            ...
            if (!res.ok) {
                showToast({ type: 'error', message: 'Falha ao carregar detalhes do grupo.' });
                return;
            }
            const data = await res.json();
            ...
            showToast({ type: 'error', message: 'Erro ao carregar detalhes do grupo.' });
        }
    };
    ...
    const handleSaveContacts = async (groupId: number, contactIds: number[]) => {
        try {
            ...
            const data = await res.json().catch(() => ({}));
            if (!res.ok) {
                showToast({ type: 'error', message: data.error || 'Falha ao salvar contatos do grupo.' });
                return;
            }

            showToast({ type: 'success', message: 'Contatos do grupo atualizados com sucesso.' });
            fetchGroups();
        } catch (err) {
            console.error('Erro ao salvar contatos:', err);
            showToast({ type: 'error', message: 'Erro ao salvar contatos do grupo.' });
        }
    };
            ...
            <main className="flex-1 p-6 bg-transparent min-h-screen">
                <div className="max-w-4xl mx-auto w-full px-2 sm:px-4">
                    <div className="flex items-center justify-between mb-6">
                        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Grupos de Contatos</h1>
                        <button
                            type="button"
                    ...
                    <div className="bg-white rounded-lg shadow-md border border-gray-200">
                        {loading ? (
                            <div className="p-6 text-center text-gray-500">Carregando grupos...</div>
                        ) : groups.length === 0 ? (
                            <div className="p-6 text-center text-gray-500">
                                {search ? "Nenhum grupo encontrado com essa busca." : "Nenhum grupo criado ainda."}
                                {!search && (
                                    <div className="mt-4">
                                        <button
                                            type="button"
