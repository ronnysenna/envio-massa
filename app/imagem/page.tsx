...
export default function ImagemPage() {
    ...
    const handleUpload = async () => {
        if (!imageFile) {
            toast.showToast({ type: "error", message: "Selecione uma imagem antes de enviar." });
            return;
        }
        ...
        try {
            ...
            toast.showToast({ type: "error", message: "Erro no upload. Tente novamente." });
        ...
        <main className="flex-1 p-8 bg-transparent min-h-screen">
            <div className="max-w-4xl mx-auto px-4">
                ...
                <div className="card max-w-lg mb-6">
                    <div className="p-6">
                        <label htmlFor="image-upload" className="block text-sm font-semibold text-gray-700 mb-2">Selecione uma imagem para upload</label>
                        <label className="cursor-pointer">
                            <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 hover:border-blue-500 transition text-center">
                                ...
                                <p className="text-gray-700 font-semibold mb-1">Clique para selecionar imagem</p>
                        ...
                        {status.type && (
                            <div className={`mt-6 p-4 rounded-lg flex items-start gap-3 ${status.type === "success" ? "bg-green-50 border border-green-200" : "bg-red-50 border border-red-200"}`}>
                                {status.type === "success" ? (
                        ...
                        {imagePreview && (
                            <div className="mt-6">
                                ...
                                <div className="relative max-w-full aspect-w-1 aspect-h-1">
                                    <Image src={imagePreview} alt="Preview" fill className="object-contain rounded-lg border border-gray-300" />
                                </div>
                                <div className="flex items-center gap-4 mt-2">
                                    ...
                                    <button type="button" onClick={handleUpload} disabled={uploading} className="px-3 py-2 bg-blue-600 text-white rounded-md text-sm hover:bg-blue-700 disabled:opacity-60">{uploading ? "Enviando..." : "Enviar imagem"}</button>
                                </div>
                                {uploadedUrl && (
                                    <div className="mt-3 text-xs text-gray-600">URL: <a className="text-blue-600" href={uploadedUrl} target="_blank" rel="noreferrer">{uploadedUrl}</a></div>
                                )}
                            </div>
                        )}
                    </div>
                        ...
                        >
                            ...
                            >
                                <div className="relative bg-white rounded shadow-lg overflow-hidden">
                                    <button type="button" onClick={closePreview} aria-label="Fechar" className="absolute top-2 right-2 z-10 bg-white rounded-full p-1 shadow">
                                        ✕
                                    </button>
                                    <div className="relative w-[80vw] h-[80vh] md:w-[60vw] md:h-[60vh]">
                                        <Image src={previewUrl} alt="Preview grande" fill className="object-contain bg-black" />
                                    </div>
                                </div>
                            </div>
                        </div>
